...
You can change the default behavior by updating the ros1ws line of your \okt{.bashrc} file.
...
Open the \okt{slider_launch.py} and add a line equivalent to calling \okt{ros2 run move_joint move_joint}.\\
Then, add a parameter to tell which joint should be controlled:\\\okt{parameters = \{'joint_name': 'right_e0'\}}\\
...
Change the hard-coded value to the argument one \okt{sl.arg('right_e0')} and check that the behavior is the same\\
...
Then, run the launch file with another name, for instance:\\ \okt{ros2 launch ./slider_launch.py name:=left_e0}\\
